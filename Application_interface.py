...
class GroceryManager:
    ...
    def create_frames(self):
        ...
        logo_label = tk.Label(self.header_frame, text=f"🛒 Smart Grocery Manager", font=("Helvetica", 24, "bold"),
                              fg="green", bg="alice blue")
        logo_label.pack(side="left", padx=20)

        # Logout Button
        logout_button = tk.Button(self.header_frame, text="🚪 Logout", font=("Helvetica", 12, "bold"), bg="blue2",
                                  fg="white", activebackground="white", activeforeground="blue2",
        ...
        # Product Name
        tk.Label(self.form_frame, text="🍎 Product Name:", font=("Helvetica", 12), bg="white").grid(row=0, column=0,
                                                                                                   sticky="w", pady=5)
        ...
        # Quantity
        tk.Label(self.form_frame, text="🔢 Quantity:", font=("Helvetica", 12), bg="white").grid(row=1, column=0,
                                                                                               sticky="w", pady=5)
        ...
        # Category Dropdown
        tk.Label(self.form_frame, text="📦 Category:", font=("Helvetica", 12), bg="white").grid(row=2, column=0,
                                                                                               sticky="w", pady=5)
        self.category_entry = ttk.Combobox(self.form_frame, font=("Helvetica", 12), state="readonly",
                                           values=["Vegetables", "Fruits", "Dairy", "Cereals", "Snacks", "Beverages",
                                                   "Others"])
        self.category_entry.grid(row=2, column=1, padx=10, pady=5, sticky="ew")
        self.category_entry.current(0)

        # Purchase Date
        tk.Label(self.form_frame, text="📅 Purchase Date:", font=("Helvetica", 12), bg="white").grid(row=3, column=0,
                                                                                                    sticky="w", pady=5)
        self.purchase_date_entry = DateEntry(self.form_frame, date_pattern="yyyy-mm-dd", font=("Helvetica", 12), bd=2,
                                             relief="sunken", bg="#F9F9F9")
        self.purchase_date_entry.grid(row=3, column=1, padx=10, pady=5, sticky="ew")

        # Expiry Date
        tk.Label(self.form_frame, text="⏳ Expiry Date:", font=("Helvetica", 12), bg="white").grid(row=4, column=0,
                                                                                                  sticky="w", pady=5)
        self.expiry_date_entry = DateEntry(self.form_frame, date_pattern="yyyy-mm-dd", font=("Helvetica", 12), bd=2,
                                           relief="sunken", bg="#F9F9F9")
        self.expiry_date_entry.grid(row=4, column=1, padx=10, pady=5, sticky="ew")

        # Notify Days
        tk.Label(self.form_frame, text="🔔 Notify Before (days):", font=("Helvetica", 12), bg="white").grid(row=5,
                                                                                                           column=0,
                                                                                                           sticky="w",
                                                                                                           pady=5)
        ...
        "ID", "Product Name", "Quantity", "Category", "Purchase Date", "Expiry Date", "Notify Days"), show="headings")
    ...
    def load_background_image(self):
        try:
            self.bg_image = Image.open(
                "/Users/arpit_lal/Desktop/Free-vector-watercolor-background-Wallpaper.jpg")  # Replace with your image path
    ...
    def load_groceries(self):
        try:
            ...
            cursor.execute(
                "SELECT id, product_name, quantity, category, purchase_date, expiry_date, notify_before FROM groceries WHERE user_id = %s",
                (self.user_id,))
    ...
    def update_grocery(self):
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE groceries SET product_name=%s, quantity=%s, category=%s, purchase_date=%s, expiry_date=%s, notify_before=%s WHERE id=%s AND user_id=%s",
                (self.product_name_entry.get(), self.quantity_entry.get(), self.category_entry.get(),
                 self.purchase_date_entry.get(), self.expiry_date_entry.get(), self.notify_before_entry.get(),
                 grocery_id, self.user_id))
            self.conn.commit()
